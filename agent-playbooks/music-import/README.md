# 批量音乐导入 Agent 指南

本文档记录了将外部音乐文件批量导入 Shining-music 系统的完整流程、API 调用方式和已知的坑。  
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
适用场景：Agent 自主完成"下载 → 建歌手 → 建歌曲 → 传音频 → 传封面 → 传歌词 → 同步 ES"全链路。

---
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

## 0. 前置准备

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
### 0.1 系统架构概要

```
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
客户端 → gateway-service (:8080) → music-service (:8082) / user-service (:8081)
                                         ↓
                              MySQL / MinIO / ES / RabbitMQ (Docker)
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```

- **Gateway 路由规则**：`/api/music/**` → music-service（StripPrefix=1，即去掉 `/api`），`/api/user/**` → user-service
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- **所有外部 API 调用统一走 gateway**: `http://localhost:8080/api/...`
- **JDK 版本**：统一使用 JDK 18（例如：`~/.local/java/jdk-18/bin/java`）

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
### 0.2 认证

所有 `/api/music/**` 接口均需 JWT（`/api/user/login` 和 `/api/user/register` 除外）。
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

```
POST http://localhost:8080/api/user/login
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
Content-Type: application/json
Body: { "username": "admin", "password": "xxx" }

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
→ 成功响应:
{
  "code": 200,
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  "message": "登录成功",
  "type": "RESULT_SUCCESS",
  "passed": true,
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  "data": { "token": "eyJhbGci...", ... }
}
```
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

后续所有请求携带 `Authorization: Bearer <token>`。

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
**统一响应格式**（所有 API 返回此结构）：

```json
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
{
  "code": 200,          // 200=成功, 500=错误
  "message": "描述信息",
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  "type": "RESULT_SUCCESS",
  "passed": true,       // 关键字段：判断操作是否成功
  "data": { ... }       // 业务数据，可能为 null
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
}
```

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
### 0.3 服务确认

确保以下服务正在运行，否则先启动：
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

| 服务 | 端口 | 检查方式 |
|------|------|----------|
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| gateway-service | 8080 | `curl localhost:8080/api/music/test` |
| music-service | 8082 | 日志或 Nacos 注册 |
| user-service | 8081 | Nacos 注册 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| MySQL | 3306 | `docker ps \| grep shining-mysql` |
| MinIO | 9000 | `curl localhost:9000/minio/health/live` |
| Elasticsearch | 9200 | `curl localhost:9200` |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| RabbitMQ | 5672 | `docker ps \| grep shining-rabbitmq` |
| Nacos | 8848 | `curl localhost:8848/nacos` |

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
**启动脚本**: `agent-playbooks/docker/start-all.sh`

如果启动失败的常见原因：
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- **Address already in use**: 用 `lsof -i :端口` 找到旧进程并 kill
- **music-service 启动报 UnsupportedClassVersionError**: 没用 JDK 18
- **common 模块找不到**: 先在项目根目录执行 `cd common && mvn install -DskipTests`
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- **ES 启动报 AccessDeniedException**: `chmod 777 docker-data/elasticsearch/`

### 0.4 数据库信息
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

| 数据库 | 说明 |
|--------|------|
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| `shining-music` | 歌曲、歌手、歌词、歌单 |
| `shining-user` | 用户 |
| `shining-community` | 社区 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| `shining-statistics` | 统计 |

连接方式：`docker exec shining-mysql mysql -u root -ppassword "shining-music"`  
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
（密码在 Nacos `secret.yaml` 中配置，默认 `password`）

关键表结构：
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- `songs`: id, title, artist_id, album_id, file_url, cover_url, duration, status, genre, release_year
- `singers`: id, name, sex, profile, genre, country, status, avatar_url
- `lyrics`: id, song_id, language_msg, content
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

### 0.5 依赖工具

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 工具 | 用途 | 安装方式 |
|------|------|----------|
| Python 3.8+ | 脚本运行 | 系统自带或 `apt install python3` |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| requests | HTTP 请求 | `pip install requests` |
| yt-dlp | Bilibili 视频下载 | `pip install yt-dlp` |
| ffmpeg | 音频提取/转码 | `apt install ffmpeg` 或通过 imageio-ffmpeg |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| mutagen | MP3 元数据读取 | `pip install mutagen` |

---
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

## A. 获取音频文件

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
系统提供两套下载脚本，按优先级使用：

### A.1 Bilibili 下载（推荐）— `agent-playbooks/music-import/bili-music-download.py`
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

从 Bilibili 搜索歌手 MV 视频，提取高质量音频。

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```bash
# 下载周杰伦的 20 首歌
python3 agent-playbooks/music-import/bili-music-download.py "周杰伦" --count 20
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

# 下载自定义歌曲列表
python3 agent-playbooks/music-import/bili-music-download.py "周杰伦" --songs my_songs.txt --count 30
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

# 下载其他歌手（自动通过酷我发现歌曲列表）
python3 agent-playbooks/music-import/bili-music-download.py "五月天" --count 15
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```

**输出目录**: `agent-playbooks/download/{歌手名}_bili/`  
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
**文件命名**: `歌手名 - 歌曲名.mp3`

#### 工作原理
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

1. 使用内置热门歌曲列表（周杰伦）或通过酷我 API 自动发现歌曲
2. 对每首歌在 Bilibili 搜索多种关键词组合：`{artist} {song} MV`、`{artist} {song} 官方`
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
3. 对每个搜索结果评分（标题匹配 +50、歌手匹配 +20、官方/高清 +15、播放量加成、时长合理性）
4. 自动过滤：现场版、翻唱、教学、伴奏、鬼畜、合唱等
5. 选取最高分视频，用 yt-dlp 下载后 ffmpeg 提取 MP3
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

#### 已知问题

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 问题 | 解决方案 |
|------|----------|
| Bilibili 412 限流 | 脚本内置重试（3 次，间隔递增），每首歌之间固定 2 秒间隔 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 下载结果不佳（翻唱/不完整） | 调整 `EXCLUDE_TITLE_RE` 正则、提高 `score` 阈值（默认 30） |
| 非周杰伦歌手没有内置歌曲列表 | 传 `--songs` 参数指定文件，或让脚本自动走酷我发现 |
| yt-dlp 版本过旧导致下载失败 | `pip install --upgrade yt-dlp` |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 通用歌名（如"青春""宿敌"）搜索匹配错误 | 搜索结果可能匹配到无关视频。Agent 必须验证视频标题确实包含目标歌曲名和歌手名 |
| 下载了 MV 版而非录音室版 | MV 视频通常有前后空白段（5:30 vs 4:28）。优先搜索"无损音质"或"完整版"而非"MV"关键词 |
| 歌曲全名与简称不一致 | 如"天龙八部之宿敌"简称"宿敌"。导入时使用完整名称，搜索时也要用完整名称 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

#### ⚠️ EXCLUDE_RE 必须覆盖的关键词（教训总结）

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
Bilibili 视频筛选时，以下关键词**必须**被排除，否则会导入错误音频：

```python
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
EXCLUDE_RE = re.compile(
    r"(?i)(live|演唱会|现场|concert|翻唱|cover|教学|教程|钢琴版|钢琴改编|吉他|伴奏"
    r"|instrumental|karaoke|反应|reaction|混剪|合集|\bAI\b|鬼畜|搞笑|舞蹈|choreography"
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    r"|弹幕|切片|手势舞|指弹|鼓谱|鼓|drum|bass|TAB|谱"
    r"|踊ってみた|歌ってみた|叩いてみた|弾いてみた"                # 日语翻跳/翻唱/翻奏
    r"|舞萌|maimai|音游|Phigros|Arcaea|osu|rhythm|beatmania"     # 音游视频
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    r"|English\s*Ver|英文版|英语版|粤语版|国语版"                   # 非原版语言
    r"|Nightcore|bootleg|sped up|slowed|8D|ASMR|耳机|录音棚"
    r"|编舞|原创编舞|口琴|尤克里里|ukulele|单簧管|黑管)")
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```

**已发生的事故**：
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- 下载了「舞萌(maimai)音游」视频当作残響散歌原曲 → 音频含游戏音效，时长不对
- 下载了「踊ってみた」舞蹈翻跳视频当作カタオモイ → 舞蹈剪辑版，时长从5:40缩到3:48
- 下载了「English Ver.」英文版当作アイドル日文版 → 语种完全错误
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- 下载了「录音棚大声听」二创视频 → 可能有额外音频处理

### A.2 酷我下载（备选，仅中文歌曲）— `agent-playbooks/music-import/music-download.py`
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

从酷我音乐搜索并直接下载 MP3。**仅适用于中文歌曲**。

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```bash
python3 agent-playbooks/music-import/music-download.py "周杰伦" --studio-only --pages 3
```
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

#### 已知限制

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- 部分歌曲只有 30 秒试听片段（付费限制）
- 需要 `--studio-only` 过滤现场版/合唱版
- 音质不如 Bilibili MV 提取
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- **⚠️ 不适用于日文/外文歌曲**：酷我的日文歌库覆盖率低，搜索结果大量混入中文翻唱版、KTV 版、错误匹配。实测表现极差，已被确认不可用于日文歌曲发现

### A.3 音频文件质量检查

> **⚠️ 音质铁律：在满足音源要求（原版、非翻唱、非剪辑、时长正确）的前提下，必须穷尽一切手段获取最高音质。** 能拿到 320kbps 就绝不接受 192kbps，能拿到 192kbps 就绝不接受 128kbps。"128kbps 合格"只是底线不是目标——找到一个 128kbps 的源不代表可以停下来，必须继续尝试其他平台/关键词寻找更高品质的源。下载时始终使用 `yt-dlp --audio-quality 0`，转码时始终使用 `-b:a 320k`。只有在所有可用平台（Bilibili、NetEase、酷我）都已尝试、确认无法获得更高品质时，才可接受当前品质。

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

下载完成后需要检查：

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```python
from mutagen.mp3 import MP3
audio = MP3(filepath)
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
duration = audio.info.length
bitrate = audio.info.bitrate // 1000
filesize = os.path.getsize(filepath)
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

if duration < 60:
    print(f"WARNING: {filename} 时长仅 {duration:.0f}s，可能是试听片段")
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
if bitrate < 128:
    print(f"WARNING: {filename} 比特率仅 {bitrate}kbps，音质较低")
```
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

**⚠️ 时长比对是必需的**。对每首歌，Agent 必须知道原版时长并与实际下载时长比较。偏差超过 30 秒说明下载了错误版本（剪辑版、片段、音游截取等）。可以通过 NetEase 搜索结果获取原版时长。

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

### A.4 音源替换搜索策略（经验总结）

当需要替换库中低品质音源时，搜索策略必须分级降级，不能在同一关键词思路上反复尝试。

#### 分级搜索策略（按顺序执行，无果后降级）

| 级别 | 关键词组合 | 示例 |
|------|-----------|------|
| 1. 精确搜索 | `{歌手名} {歌名}` | `陈昕桐 はじまりの歌` |
| 2. 歌名+中文译名 | `{歌名} {中文译名}` | `はじまりの歌 初始之歌` |
| 3. 纯歌名+用途词 | `{歌名} 原版/音源/MV/歌手` | `はじまりの歌 原版 MV` |
| 4. 作品名/动漫名关联 | `{关联作品名} {歌名}` | `亚人酱有话要说 フェアリーテイル` |
| 5. 翻唱/推荐版本 | 放宽到非原唱但高质量版本 | `はじまりの歌 歌曲 推荐` |

**铁律**：精确搜索（级别 1）连续 2 次无果后，**必须立即降级**到纯歌名搜索（级别 2-3），而不是换同级别的关键词组合继续试。

#### B站视频时长 ≠ 实际音频时长

B站视频列表中显示的"4:02"是**视频时长**，不是音频时长。常见差异原因：
- 视频有静音开头/结尾画面（歌曲推荐类视频常见）
- 视频标注包含了前后字幕/弹幕时间
- 多 P 视频只显示第一 P 时长

**必须在下载后用 ffmpeg 验证实际音频时长**，不能信任搜索结果中的 `duration` 字段。

#### B站搜索盲区（已发生的事故）

| 场景 | 原因 | 教训 |
|------|------|------|
| 搜 `三月のパンタシア フェアリーテイル` 无结果 | 视频标题用动漫名"亚人酱有话要说"而非歌手名 | 歌手名搜不到就试作品名/动漫名 |
| 搜 `陈昕桐 はじまりの歌` 只找到 2:43 短版 | 完整版视频标题是"推荐歌曲《はじまりの歌》"，不含歌手名 | 纯歌名搜索可以覆盖更多上传者 |
| YouTube 全站被 SABR 机制阻断下载 | yt-dlp 无 cookies 时 403 Forbidden | YouTube 不可作为可靠音源，优先用 B站 |
| Niconico 网络不通 | 环境限制，Network is unreachable | 当前环境不可用 |

#### 音源质量审计方法

批量检查库中所有歌曲的音频质量：

```bash
# 用 ffmpeg 探测 MinIO 远程文件的码率和时长
FFMPEG=/home/chenxinyao/.local/lib/python3.8/site-packages/imageio_ffmpeg/binaries/ffmpeg
$FFMPEG -i "<minio_file_url>" 2>&1 | grep -E 'Duration|bitrate|Audio'
```

质量分级标准：
- **高品质**: bitrate >= 192 kb/s → **这才是目标水平**
- **合格但不满足**: 128 kb/s <= bitrate < 192 kb/s → 底线，必须继续尝试其他平台寻找更高品质源，所有平台都试过才可接受
- **低品质**: bitrate < 128 kb/s → **必须替换，无例外**
- **极低品质**: bitrate < 10 kb/s → 大概率是损坏文件或占位符

> **注意**：AAC 编码效率高于 MP3，~110kbps AAC ≈ ~130-140kbps MP3。对 AAC/m4a 文件，需用容器 bitrate（非 stream bitrate）评估质量。

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
---

## B. 获取歌词
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

### B.1 NetEase 歌词获取（推荐）

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
NetEase 是最可靠的 LRC 歌词来源，大多数中文歌曲都有带时间戳的歌词和翻译。

#### 步骤 1：获取 NetEase song_id
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

```python
r = requests.get("https://music.163.com/api/search/get",
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    params={"s": f"{artist} {title}", "type": 1, "limit": 5},
    headers={"User-Agent": "Mozilla/5.0", "Referer": "https://music.163.com/"})
songs = r.json().get("result", {}).get("songs", [])
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
# 遍历 songs，匹配 artists[].name 和 name 字段
```

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
**注意**：搜索结果可能不返回原版歌曲（被 remix/cover 淹没），需要验证歌手名匹配。

#### 步骤 2：获取歌词内容
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

```python
r = requests.get(f"https://music.163.com/api/song/lyric?id={netease_id}&lv=1&kv=1&tv=-1",
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    headers={"User-Agent": "Mozilla/5.0", "Referer": "https://music.163.com/"})
data = r.json()

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
original_lrc = data.get("lrc", {}).get("lyric", "")       # 原文歌词
translation  = data.get("tlyric", {}).get("lyric", "")     # 翻译歌词（如有）
```
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

#### 步骤 3：歌词格式验证

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
下载后**必须**检查歌词质量，因为以下格式不可用：

```python
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
# 检测 QRC/KRC 逐字格式（会导致中文乱码）
if "<" in lrc_content and ">" in lrc_content:
    print("BAD: QRC/KRC karaoke format detected, discard and refetch")
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

# 检测乱码
if lrc_content.count("?") > len(lrc_content) * 0.3:
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    print("BAD: Garbled content detected")

# 检测有效行数
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
import re
valid_lines = [l for l in lrc_content.split('\n')
    if re.match(r'\[\d{2}:\d{2}', l)
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    and not any(kw in l.lower() for kw in ['作词', '作曲', '编曲', '制作人', 'op ', 'isrc', 'ar:', 'ti:', 'al:'])]
if len(valid_lines) < 10:
    print(f"BAD: Only {len(valid_lines)} valid lines")
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```

### B.2 lrclib.net（备选）
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

适用于 NetEase 上找不到歌词的情况（尤其是外文歌曲）。

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```python
r = requests.get("https://lrclib.net/api/get",
    params={"artist_name": artist, "track_name": title},
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    headers={"User-Agent": "ShiningMusic/1.0"})
if r.status_code == 200:
    data = r.json()
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    synced_lrc = data.get("syncedLyrics", "")   # 带时间戳
    plain_lrc  = data.get("plainLyrics", "")     # 纯文本（无时间戳，不推荐）
```
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

### B.3 歌词语言标识规范

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
上传歌词时 `msg` 参数的语言标识：

| 语言 | msg 值 | 说明 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
|------|--------|------|
| 中文 | `zh` | 包括简体/繁体 |
| 日语 | `ja` | |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 英语 | `en` | |
| 韩语 | `ko` | |
| 翻译（中→其他） | 目标语言代码 | 如日文歌的中文翻译用 `zh` |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

---

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
## 1. 创建/查找歌手

### ⛔ 强制规则：必须使用 `create_singer.py` 脚本创建歌手

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

**禁止用 SQL INSERT 直接创建歌手记录。** 直接 SQL 会绕过去重检查、跳过头像上传、遗漏 profile/genre/country，导致残缺歌手记录。

**唯一合法方式**：

```bash
python3 agent-playbooks/music-import/create_singer.py \
  --name "Eve" --sex 1 \
  --profile "日本男性歌手、音乐创作人。代表作廻廻奇譚等。" \
  --genre "J-Pop/Rock/Anime" --country "日本" \
  --netease-id 1075075
```

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

该脚本强制执行以下全部步骤，任何一步失败则报错退出：
1. 检查歌手是否已存在（去重）
2. 通过 API 创建歌手（status=0 活跃）
3. 通过 API 补全 profile/genre/country
4. 从 NetEase 获取头像并上传（验证文件 >5KB）
5. **验证所有字段完整性**（avatar_url/profile/genre/country/status 全部非空）

**⚠️ 歌手 status 含义与歌曲不同**：歌手 `status=0` 表示"活跃"，`status=1` 表示"停更/退役"。歌曲 `status=1` 表示"正常可用"，`status=0` 表示"禁用"。两者含义相反，切勿混淆。

**⚠️ singers.sex 标注标准（铁律）**：`sex` 代表**歌曲中主唱人声的性别**，不是歌手/组合的组成方式。

- 组合/乐队：按**主唱**性别标注（YOASOBI 主唱 ikura 是女性 → sex=1，King Gnu 主唱井口理是男性 → sex=0）
- 制作人/DJ：按歌曲中**实际演唱者**的性别标注（HoneyWorks 歌曲由女性歌手演唱 → sex=1）
- sex=2 **仅用于**歌曲确实是男女合唱的情况，不得用于"这是一个组合所以不确定"的偷懒标注
- 创建歌手时如果不确定主唱性别，**必须上网查清楚再填**，禁止默认填 sex=2

详细规则见 `agent-playbooks/song-tagging/README.md` Step 2: Vocal 章节。

**注意**：singers.sex 仍需正确填写（用于元数据展示），但 Vocal 标签已改为 SVM 音频分析产生浮点值，不再使用 singers.sex 的二值映射。

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

### 2026-04-10 事故记录

5 个新歌手（Eve/藤井風/King Gnu/Official髭男dism/米津玄師）通过 SQL INSERT 直接创建，导致：
- avatar_url 全部 NULL（无头像）
- profile 全部 NULL（无简介）
- country 用了英文 "Japan" 而非中文 "日本"
- status=1（退役）而非 status=0（活跃）
- genre 格式与现有记录不一致

**根因**：绕过了 API 和 `create_singer.py` 的校验链路，直接操作数据库。

### ⚠️ 歌手存在性检查规范（创建前必须完成）

仅靠精确 name 匹配远远不够。创建歌手前，必须按以下顺序逐项排查：

1. **精确匹配**：`SELECT id, name FROM singers WHERE name = '歌手名';`
2. **模糊匹配**：`SELECT id, name FROM singers WHERE name LIKE '%关键词%';`（处理别名、简称、外文名等）
3. **变体检查**：全角/半角（Ｈｏｎｅｙ vs Honey）、简繁体（遊 vs 游）、大小写、带不带句号/破折号
4. **合作歌曲拆分检查**：歌曲标注 "A/B" 或 "A feat. B" 时，必须分别查 A 和 B 是否已存在。系统约定主歌手归 singers 表，feat./合作信息保留在歌曲标题中
5. **NetEase 歌手 ID 交叉验证**：用 NetEase API 获取歌手 ID，检查 DB 中是否已有使用同一 NetEase ID 创建的歌手（可能名字不同但实际是同一人）

只有以上 5 项全部排查无命中，才可创建新歌手。

### 已知问题与注意事项

| 问题 | 原因 | 解决方案 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
|------|------|----------|
| `Column 'status' cannot be null` | 后端 createSinger 曾不设默认值 | **已修复**：status 默认 0（活跃），sex 默认 0。但建议仍显式传值 |
| 歌手重复创建 | 同一歌手可能有多种名称（真名/艺名/外文名） | `create_singer.py` 内置去重检查，但 Agent 仍须自行完成上述 5 项排查 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 歌手信息不全 | 仅传了名字，没有性别/简介/头像 | `create_singer.py` 强制所有参数必填 |
| 直接 SQL 创建歌手 | 绕过校验链路 | **已禁止**。必须使用 `create_singer.py` |

### API（仅供 create_singer.py 内部使用）

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

### 更新歌手详细信息
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

```
POST /api/music/update-profile
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
Body: { "id": <singerId>, "sex": 0, "profile": "...", "genre": "流行", "country": "中国" }
```

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
### 上传歌手头像

```
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
POST /api/music/update-avatar
Form: id=<singerId>, avatarFile=<file>, md5=<file_md5>
```
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

**头像来源**: NetEase 歌手详情页 → `artist.img1v1Url` 或 `artist.picUrl`。

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
---

## 2. 创建歌曲记录
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

### API

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```
POST /api/music/song
Body: { "title": "七里香", "artistId": <singerId>, "status": 1 }
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
→ data.id （新创建的 song_id）
```

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
### 注意

- `title` 从文件名提取：去掉扩展名，按 ` - ` 分割取后半部分
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- `status: 1` 表示正常可用
- 返回的 `song_id` 后续所有操作都会用到，务必保存

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
### 补充字段（创建后更新）

歌曲创建后，**必须**通过 SQL 补全以下字段：
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

```sql
UPDATE songs SET genre = 'J-Pop', release_year = 2023, duration = 226 WHERE id = <song_id>;
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```

| 字段 | 说明 | 必填 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
|------|------|------|
| `genre` | 音乐流派（J-Pop, J-Rock, Electropop, Pop, Rock 等） | ✅ |
| `release_year` | 发行年份（用于 Era 标签计算） | ✅ |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| `duration` | 时长（秒），从 mutagen 读取 | ✅ |
| `volume_gain` | 响度补偿增益（dB），uploadSong 上传时自动计算 | 自动 |
| `lufs` | 积分响度（LUFS），uploadSong 上传时自动写入 | 自动 |

不填 `genre` 和 `release_year` 会导致标签系统的 Source 和 Era 维度无法自动补全。
`volume_gain` 和 `lufs` 由 uploadSong API 自动填充，无需手动设置。

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

### volume_gain 与 LUFS（响度归一化）

前端播放器通过 WebAudio GainNode 应用 `volume_gain`（dB），使所有歌曲播放响度趋于一致。

**自动处理**：`uploadSong` API 上传音频时，后端 `LufsAnalyzer` 自动用 ffmpeg loudnorm 测量积分响度（LUFS），写入 `songs.lufs` 列，并以当前库均值 LUFS 为目标计算 `volume_gain = AVG(lufs) - lufs`。**新导入的歌曲不需要手动计算 volume_gain。**

**全库重算 API**：当库均值因新歌加入发生漂移时，调用 `POST /music/recalc-volume-gain`（直连 8082）或 `POST /api/music/recalc-volume-gain`（走 gateway，需鉴权）可一键重算所有歌曲的 volume_gain。

**手动测量**（仅在音源不经过 uploadSong API 时需要）:

```bash
# 测量音频的积分响度
ffmpeg -hide_banner -nostats -i "<audio_file_or_url>" -af loudnorm=print_format=json -f null /dev/null 2>&1
# 输出 JSON 中的 "input_i" 字段即为 LUFS 值
# volume_gain = 库均值LUFS - input_i_lufs
```

**⚠️ 替换音源后 volume_gain 会自动更新**（通过 uploadSong API 上传时），无需手动处理。

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

---

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
## 3. 上传音频文件

### API
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

```
POST /api/music/upload
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
Form: id=<songId>, avatarFile=<audio_file>, md5=<file_md5>
```

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
### 注意

- 参数名是 `avatarFile`（历史命名，实际传音频文件）
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- `md5` 是音频文件的 MD5 校验值
- Content-Type 用 `audio/mpeg`

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
---

## 4. 上传歌曲封面（重点难点）
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

### API

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```
POST /api/music/cover/song
Form: id=<songId>, avatarFile=<image_file>, md5=<file_md5>
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```

### 封面获取策略（按优先级排序）
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

**核心原则：每首歌的封面必须是独一无二的图片，绝不能多首歌共用同一张图。**

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 优先级 | 来源 | 方法 | 独特性 |
|--------|------|------|--------|
| 1 | **Bilibili MV 缩略图** | 搜索 `{artist} {title} MV` → 取视频 `pic` 字段 | ✅ 每个 MV 视频封面天然不同 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 2 | **NetEase MV 封面** | `GET /api/search/get?type=1004&s={artist}+{title}` → `mvs[].cover` | ✅ MV 封面独特 |
| 3 | **QQ Music MV 封面** | `search_type=12` 搜 MV → `mv_vid` → `T015R300x300M000{vid}.jpg` | ✅ MV 封面独特 |
| ❌ | ~~专辑封面~~ | 无论来自 NetEase/QQ Music/Apple Music | ❌ 同专辑歌曲会共用 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

### 已踩的坑

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 错误做法 | 后果 | 正确做法 |
|----------|------|----------|
| 用 NetEase 搜索 API 的 `album.picUrl` | 同专辑歌曲共享同一封面，用户不满意 | 用 MV 封面 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 用 QQ Music 的 `album.mid` 构造封面 URL | 同上 | 搜 MV（type=12）而非歌曲（type=0） |
| 不检查封面是否重复 | 上传了重复图片浑然不知 | 维护 `seen_urls` 集合做去重检测 |
| Bilibili API 连续请求 | 触发 412 限流 | 每次请求间隔 2-3 秒，412 时指数退避 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 封面图片太小（< 5KB） | 可能是占位图/404 页面 | 检查下载文件大小，最低 5KB |

### Bilibili 搜索防限流模板（Cookie 方案）
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

**核心原理**：Bilibili 412 限流的根因是无 cookies 的匿名请求被识别为爬虫。通过先访问主页获取 `buvid3`/`b_nut` cookies，再调用 SPI 接口获取指纹 cookies，即可建立有效的匿名会话，大幅降低 412 概率。

```python
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
import requests, time

def create_bili_session():
    """创建带有效 cookies 的 Bilibili session，避免 412 限流"""
    session = requests.Session()
    session.headers.update({
        'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
        'Referer': 'https://www.bilibili.com/',
        'Accept': 'application/json, text/plain, */*',
    })
    # Step 1: 访问主页获取 buvid3, b_nut cookies
    session.get('https://www.bilibili.com/', timeout=10)
    # Step 2: 获取 SPI 指纹（进一步降低风控概率）
    session.get('https://api.bilibili.com/x/frontend/finger/spi', timeout=10)
    return session

def search_bilibili(session, keyword, retries=3):
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    for attempt in range(retries):
        time.sleep(2)  # 每次请求间隔 2s
        r = session.get("https://api.bilibili.com/x/web-interface/search/type",
            params={"search_type": "video", "keyword": keyword, "page": "1"},
            timeout=15)
        if r.status_code == 412:
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
            time.sleep((attempt + 1) * 10)  # 10s, 20s, 30s
            session = create_bili_session()  # 重新创建 session
            continue
        return r.json().get("data", {}).get("result", [])
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    return []

# 使用方式：
# session = create_bili_session()
# results = search_bilibili(session, "歌手名 歌曲名")
```

> **经验总结**（2026-04-17）：不带 cookies 的纯 requests.get 在连续搜索 3-5 次后必定触发 412，sleep 再久也没用。使用 cookie 方案后，同一 session 可稳定搜索 20+ 次不触发限流。如果 session 过期（超过 30 分钟），重新调用 `create_bili_session()` 即可。

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
### 验证封面独特性

上传完成后，必须检查所有封面的文件大小是否都不同：
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

```python
for song_id in all_ids:
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    r = requests.head(cover_url)
    size = int(r.headers.get('Content-Length', 0))
    # 记录 {song_id: size}，检查 size 集合长度 == 歌曲数
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```

---
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

## 5. 上传歌词及验证（重点难点）

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
> 歌词的**获取方法**见 [Section B](#b-获取歌词)，本节聚焦**上传 API 和质量验证**。

### API
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

```
POST /api/music/lyrics/{songId}
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
Form: lyricsFile=<lrc_file>, msg=<language_code>
```

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- `msg` 参数是语言标识（如 `zh`、`ja`、`en`）
- 同一首歌同一语言重复上传会自动覆盖旧歌词

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
### 歌词获取来源

| 来源 | 方法 | 质量 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
|------|------|------|
| **NetEase（推荐）** | `GET /api/song/lyric?id={netease_id}&lv=1&kv=1&tv=-1` → `lrc.lyric` + `tlyric.lyric` | ✅ 标准 LRC，有翻译 |
| lrclib.net | `GET /api/get?artist_name={}&track_name={}&album_name={}` | 一般，可能缺少 |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| QQ Music | 需要特殊解密 | 不推荐 |

### 获取 NetEase song_id
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

```python
r = requests.get("https://music.163.com/api/search/get",
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    params={"s": f"{artist} {title}", "type": 1, "limit": 5},
    headers={"User-Agent": "Mozilla/5.0", "Referer": "https://music.163.com/"})
# 从 result.songs 中匹配 artist + title
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```

### 已踩的坑
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

| 错误做法 | 后果 | 正确做法 |
|----------|------|----------|
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 直接使用搜索结果的歌词而不检查 | 部分歌词是 QRC/KRC 逐字格式（`<timestamp>` 标签），中文变成 `?` | 下载后检查是否包含 `<` 标签和 `?` 乱码 |
| 歌词上传后不验证 | 3/20 首歌只有 3-4 行废数据 | 上传后必须审计：检查有效行数（排除元数据行） |
| 传参用 `lang` | 400 Bad Request | 参数名是 `msg` |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 不获取翻译歌词 | 用户希望双语对照 | `tlyric.lyric` 是翻译，也要上传（msg 设为对应语言） |
| **用 `json.dumps()` 转义歌词内容后 SQL INSERT** | **unicode 字符变成 `u4f5c` 等 ASCII 垃圾（默认 ensure_ascii=True 产生 \uXXXX，经 shell 丢失 backslash）** | **必须用 `fix_lyrics.py`（stdin pipe + escape_sql）或 API 上传，禁止 json.dumps 做 MySQL 转义** |
| **插入后只查 COUNT(*) 不查内容** | **20 首歌歌词全部是乱码却"验证通过"** | **每首歌插入后必须 `SELECT LEFT(content, 50)` 确认显示的是真实文字** |

> **2026-04-10 歌词事故记录**：第二批 20 首歌（ID 298-317）全部使用 `json.dumps()` + shell + SQL INSERT 写入歌词，导致所有 unicode 字符变成 `u4f5c` 等 ASCII 字面文本。根因：(1) 绕过了 README 推荐的 API 上传方式；(2) json.dumps 的 ensure_ascii=True 默认值 + shell 吞掉 backslash；(3) 验证只查行数不查内容。已用 `fix_lyrics.py` 修复全部 20 首。

### 歌词质量审计标准
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

```python
lines = content.split('\n')
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
valid = [l for l in lines if re.match(r'\[\d{2}:\d{2}', l)
         and not any(kw in l for kw in ['作词', '作曲', '编曲', '制作人', 'ar:', 'ti:', 'al:'])]
if len(valid) < 10:
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    print(f"BAD: {title} 只有 {len(valid)} 行有效歌词")
```

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
### 系统支持的 LRC 格式

后端 `LrcParser.java` 支持：
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- 标准格式：`[mm:ss.xx]歌词文本`
- 无毫秒格式：`[mm:ss]歌词文本`
- 扩展多语言格式：`[mm:ss.xx][lang]文本`
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

前端 `lrcParser.js` 额外支持：
- 空行/间奏标记：仅有时间戳无文本 → 显示为间奏
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- 元数据过滤：自动忽略作词/作曲等元信息行
- 多语言合并：`mergeMultiLang(sources)` 合并不同语言记录

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
---

## 6. 同步 Elasticsearch
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

### API

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```
POST /api/music/search/sync
→ {"message": "全量同步完成"}
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```

### 注意
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

- 每次批量导入完成后**必须**调用此接口
- 否则新歌曲在搜索中不可见
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
- 同步包括歌曲标题、歌手名、歌词内容

---
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

## 7. 完整导入流程 Checklist

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
```
□ Step 0: 环境准备
  □ 所有 Docker 容器运行中（MySQL, MinIO, ES, RabbitMQ, Redis, Nacos）
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  □ 后端服务运行中（gateway, music-service, user-service）
  □ 获取 JWT token: POST /api/user/login → data.token

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
□ Step A: 下载音频文件
  □ 运行 bili-music-download.py（或准备本地音频文件）
  □ 验证：每个 MP3 时长 > 60s、比特率 >= 192kbps（128kbps 仅为底线，必须穷尽所有平台后才可接受）、文件完整
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  □ 文件名格式：「歌手名 - 歌曲名.mp3」

□ Step 1: 创建/查找歌手 → 得到 singer_id
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  □ 存在性 5 项排查（详见 Section 1「歌手存在性检查规范」）：
    □ 1. 精确 name 匹配
    □ 2. 模糊 LIKE 匹配（别名/简称/外文名）
    □ 3. 变体检查（全角半角/简繁/大小写/标点）
    □ 4. 合作歌曲拆分检查（A/B → 分别查 A 和 B）
    □ 5. NetEase 歌手 ID 交叉验证
  □ 若不存在 → 使用 create_singer.py 创建（禁止 SQL INSERT）
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  □ 创建后验证所有字段完整（avatar_url/profile/genre/country/status）

□ Step 2: 遍历音频文件（每首歌）
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  □ 2a: POST /api/music/song 创建歌曲记录 → 保存 song_id
  □ 2b: POST /api/music/upload 上传音频（参数名: avatarFile）
  □ 2c: 搜索并上传 MV 封面
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    □ 优先 Bilibili MV 缩略图（搜 "{artist} {title} MV"）
    □ 备选 NetEase MV 封面（type=1004 搜索）
    □ 验证封面独特性（不能与已上传的重复）
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    □ 验证封面文件大小 > 5KB
    □ POST /api/music/cover/song 上传
  □ 2d: 获取并写入歌词（**必须用 fix_lyrics.py 或 API，禁止 json.dumps + SQL**）
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    □ 方式一（推荐）：`python3 agent-playbooks/music-import/fix_lyrics.py <song_id> <netease_id>`
    □ 方式二：POST /api/music/lyrics/{songId} 上传（参数: lyricsFile + msg=语言标识）
    □ **禁止方式**：json.dumps() + docker exec mysql -e INSERT（会导致 unicode 变 ASCII 垃圾）
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    □ 写入后**必须**验证内容：`SELECT LEFT(content, 50) FROM lyrics WHERE song_id=X` 确认是真实文字
    □ 如有翻译歌词（tlyric），fix_lyrics.py 会自动写入中文翻译
  □ 每完成一首歌，追加到 import_results.json
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

□ Step 3: 补全元数据
  □ UPDATE songs SET genre, release_year, duration WHERE id IN (...)
  □ volume_gain 已由 uploadSong API 自动计算写入，无需手动处理
  □ 若需全库重算（新歌导致均值漂移）：POST /music/recalc-volume-gain
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  □ 验证：SELECT id, title, genre, release_year, duration, volume_gain FROM songs WHERE ...

□ Step 4: POST /api/music/search/sync 全量同步 ES
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

□ Step 5: 歌曲标签打标（⚠️ 必须严格遵守 song-tagging/README.md）
  □ 运行 batch_tag.py 或手动逐首打标
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  □ 确认 28 维标签全部写入（Language 4 + Source 6 + Vocal 3(SVM浮点) + Audio 6 + Mood 8 + Era 1）
  □ POST /recommend/songs/rebuild-all 重建向量
  □ 验证：SELECT song_id, COUNT(*) FROM song_tags WHERE song_id IN (...) GROUP BY song_id → 每首 28
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

□ Step 6: 最终验证（逐首检查，不能跳过）
  □ DB 验证：SELECT count(*) FROM songs WHERE artist_id = <singer_id>
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  □ 封面验证：所有 cover_url 不同，HEAD 请求检查 Content-Length 各不相同
  □ 歌词验证：每首歌 lyrics 表有记录，content 有效行数 >= 10，**且 `SELECT LEFT(content, 50)` 显示真实文字（非 `u4f5c` 等 ASCII 转义）**
  □ 标签验证：每首歌 song_tags 有 28 条记录，Source 和 Era 不能全为 0
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  □ 音频验证：时长与预期偏差 < 30s，比特率 >= 128kbps
  □ ES 验证：搜索 songId 在 music_search 索引中存在
  □ 歌手验证：avatar_url 非空，sex 正确
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  □ 前端验证：歌手详情页能看到所有歌曲，播放全部功能正常
```

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
### import_results.json 格式

每首歌完成后保存进度，便于故障恢复：
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

```json
[
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  {
    "title": "七里香",
    "file": "周杰伦 - 七里香.mp3",
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    "song_id": 50,
    "audio": true,
    "cover": true,
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
    "lyrics": true,
    "netease_id": 185821,
    "cover_source": "bilibili"
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
  }
]
```
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

---

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
## 8. 错误处理原则

1. **不要静默跳过失败**：每一步失败都要记录到结果 JSON，便于后续修复
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
2. **分阶段保存进度**：每完成一首歌就将 `{song_id, title, audio, cover, lyrics}` 追加到 `import_results.json`
3. **封面和歌词问题必须单独修复脚本**：批量导入时封面/歌词最容易出错，失败的部分要能单独重跑
4. **Agent 必须参与异常处理**：纯脚本无法处理所有边界情况（API 限流、歌词格式异常、封面重复等），Agent 需要在脚本执行过程中监控输出并介入
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
5. **导入后必须逐首验证**：不能只看推荐分数。必须检查每首歌的音频是否原版、标签是否正确、字段是否完整。"看起来能跑" ≠ "质量合格"

## 8.1 关联文档（必读）
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

| 文档 | 路径 | 用途 |
|------|------|------|
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| **歌曲标签打标指南** | `agent-playbooks/song-tagging/README.md` | 28 维标签打标的完整规范，包括 Language/Source/Mood/Vocal/Audio/Era 六类 |
| **外部发现与推荐指南** | `agent-playbooks/external-discover/README.md` | 从外部平台发现歌曲、评估匹配度并导入系统的完整流程 |

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
**导入歌曲后必须完成标签打标**，否则推荐系统无法正常工作。标签打标必须严格按照 `song-tagging/README.md` 的规范执行。

---
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**

## 9. 外部 API 速率限制

> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| 平台 | 限制 | 建议间隔 |
|------|------|----------|
| NetEase Music API | 较宽松 | 0.3-0.5s |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
| Bilibili Search API | 严格，易触发 412 | 2-3s，412 后指数退避 |
| QQ Music API | 中等 | 0.5-1s |
| 本地 Gateway API | 无限制 | 0.1s |
> **!!!!! 禁止一切批量操作！每首歌必须逐首手动处理并独立验证！禁止编写或运行任何批量循环脚本！ !!!!!**
